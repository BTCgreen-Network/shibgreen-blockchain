...
get_madmax_filename() {
  ...
  echo "${SHIBGREEN_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" shibgreen_plot
  chmod 755 shibgreen_plot
  echo "Successfully installed madmax to $(pwd)/shibgreen_plot"
  ...
  mv -f "${madmax_filename}" shibgreen_plot_k34
  chmod 755 shibgreen_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/shibgreen_plot_k34"
else
  usage
fi
