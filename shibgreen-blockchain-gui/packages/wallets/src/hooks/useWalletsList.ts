...
import { useGetWalletsQuery, useGetStrayCatsQuery, useGetCatListQuery, useAddCATTokenMutation } from '@shibgreen/api-react';
import { useShowError } from '@shibgreen/core';
  ...
  const list = useMemo(() => {
    ...
    let tokens = [
      ...baseWallets.map((wallet: Wallet) => ({
        ...
        name: wallet.type === WalletType.STANDARD_WALLET ? 'SHIBgreen' : wallet.meta?.name ?? wallet.name,
      })),
