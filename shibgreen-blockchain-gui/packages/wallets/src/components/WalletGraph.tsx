import { TransactionType, WalletType } from '@shibgreen/api';
import type { Transaction } from '@shibgreen/api';
import { useGetWalletBalanceQuery } from '@shibgreen/api-react';
import { mojoToSHIBgreen, mojoToCAT, blockHeightToTimestamp } from '@shibgreen/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSHIBgreen(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToSHIBgreen(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSHIBgreen(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSHIBgreen(start).toString(), // bignumber is not supported by react
    });
  });
