import { WalletType, TransactionType, toBech32m } from '@shibgreen/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? mojoToCAT(row.amount) : mojoToSHIBgreen(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToSHIBgreen(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
