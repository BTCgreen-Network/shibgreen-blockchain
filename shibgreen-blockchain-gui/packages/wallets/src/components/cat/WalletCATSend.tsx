import { SyncingStatus, toBech32m } from '@shibgreen/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@shibgreen/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send shibgreen to coloured address. Please enter a shibgreen address.`);
    }
    ...
    const feeValue = shibgreenToMojo(fee);
