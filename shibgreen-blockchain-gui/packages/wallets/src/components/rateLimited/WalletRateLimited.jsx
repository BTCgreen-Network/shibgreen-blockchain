import { AlertDialog, Card, Flex, shibgreenToMojo, mojoToSHIBgreenLocaleString } from '@shibgreen/core';
...
function IncompleteCard(props) {
  ...
  function submit() {
    ...
    const hexshibeck = /[\da-f]+$/gi;

    if (!hexshibeck.test(ip_val) || ip_val.value === '') {
    ...
    const shibgreenper_input = ip_parsed.limit;
    ...
    const shibgreenper_value = Number.parseInt(Number(shibgreenper_input));
    const origin_parsed = JSON.parse(origin_input);
    dispatch(rl_set_user_info_action(id, interval_value, shibgreenper_value, origin_parsed, admin_pubkey_input));
  }
      ...
      <Card title={<Trans>Rate Limited Info</Trans>}>
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>Spending Limit (shibgreen per interval): {mojoToSHIBgreenLocaleString(limit)}</Trans>
              </Typography>
            </Box>
          </Box>
        </Grid>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToSHIBgreenLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props) {
  ...
  function send() {
    ...
    const amount = shibgreenToMojo(amount_input.value);
    const fee = shibgreenToMojo(fee_input.value);
