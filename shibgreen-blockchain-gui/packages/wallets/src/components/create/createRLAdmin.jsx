import { AlertDialog, shibgreenToMojo } from '@shibgreen/core';
...
export function CreateRLAdminWallet() {
  ...
  function create() {
    ...
    if (
      shibgreenper_input.value === '' ||
      Number(shibgreenper_input.value) === 0 ||
      !Number(shibgreenper_input.value) ||
      isNaN(Number(shibgreenper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const shibgreenper = shibgreenToMojo(shibgreenper_input.value);
    ...
    // var fee = shibgreenToMojo(fee_input.value);
    // TODO(lipa): send fee to server
    // const fee_value = parseInt(Number(fee));
    dispatch(create_rl_admin_action(interval_value, shibgreenper_value, userpubkey, amount_value));
  }
