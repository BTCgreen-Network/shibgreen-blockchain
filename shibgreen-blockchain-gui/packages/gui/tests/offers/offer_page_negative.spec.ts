...
test('Confirm Error message when fields are not completed on Offer Page', async () => {
  ...
  await page.locator('text=Amount *TXSHIB >> input[type="text"]').fill('0.05');
  ...
  await page
    .locator('text=Create an NFT OfferBuy an NFTSell an NFTYou will offerAmount *TXSHIB50,000,000,000 >> button')
    .first()
    .click();
  ...
  await page.locator('text=You will offerAsset Type *​Amount *TXSHIB >> input[type="text"]').fill('0.00000000005');

  // And I complete Amount under heading In exchange for
  await page.locator('text=In exchange forAsset Type *​Amount *TXSHIB >> input[type="text"]').fill('0.00000000005');
