import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@shibgreen/api-react';
import {
  UnitFormat,
  CardStep,
  ButtonLoading,
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = shibgreenToMojo(fee);
        ...
        <Flex flexDirection="column" gap={3}>
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat value={mojoToSHIBgreenLocaleString(balance)} display="inline" state={State.SUCCESS} /> to{' '}
                {address}
              </Trans>
            </Typography>
