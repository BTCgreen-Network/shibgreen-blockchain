import { type OfferSummaryRecord } from '@shibgreen/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToSHIBgreen, mojoToCAT } from '@shibgreen/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xshib', 'txshib'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToSHIBgreen(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xshib', 'txshib'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToSHIBgreen(takerEntries[0][1])
