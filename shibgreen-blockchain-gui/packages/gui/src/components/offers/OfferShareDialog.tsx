...
import { OfferTradeRecord } from '@shibgreen/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('shibgreen-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.shib.green';
...
async function postToOfferBin(offerData: string, sharePrivately: boolean, testnet: boolean): Promise<string> {
  ...
  if (testnet) {
    return 'https://www.shib.green/offers';
  }
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. XSHIB/RandoCoin
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<string> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txshib' : 'xshib'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'txshib' : 'xshib'}/offer/${id}`;
}
...
const KeybaseTeamName = 'shibgreen_offers';
...
function OfferShareKeybaseDialog(props: OfferShareServiceDialogProps) {
  ...
  const teamName = testnet ? 'testxshiboffersdev' : KeybaseTeamName;
