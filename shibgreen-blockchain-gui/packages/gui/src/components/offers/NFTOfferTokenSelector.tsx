...
import type { CATToken, Wallet } from '@shibgreen/api';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const shibgreenWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'SHIBgreen',
        symbol: currencyCode,
        displayName: `SHIBgreen (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...shibgreenWalletSelection, ...catOptions];
