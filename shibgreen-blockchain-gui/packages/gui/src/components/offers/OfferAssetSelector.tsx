import { Wallet, WalletType, type CATToken } from '@shibgreen/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@shibgreen/api-react';
import { Select, useCurrencyCode } from '@shibgreen/core';
...
type BuildAssetSelectorListParams = {
  ...
  shibgreenCurrencyCode: string;
};

function buildAssetSelectorList(params: BuildAssetSelectorListParams): WalletOfferAssetSelection[] {
  const { wallets, catList, rows, otherRows, selectedWalletId, shibgreenCurrencyCode } = params;
...
function OfferAssetSelector(props: OfferAssetSelectorProps) {
  ...
  const options: WalletOfferAssetSelection[] = useMemo(() => {
    ...
    return buildAssetSelectorList({
      ...
      otherRows,
      selectedWalletId,
      shibgreenCurrencyCode: currencyCode,
    });
