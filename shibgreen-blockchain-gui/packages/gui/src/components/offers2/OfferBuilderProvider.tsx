import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@shibgreen/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@shibgreen/api';
import { useCalculateRoyaltiesForNFTsQuery, useGetNFTsByNFTIDsQuery, useGetWalletsQuery } from '@shibgreen/api-react';
import { catToMojo, shibgreenToMojo } from '@shibgreen/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXSHIB = useWatch({
    name: 'requested.xshib',
  });

  const offeredXSHIB = useWatch({
    name: 'offered.xshib',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXSHIB ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xshib', shibgreenToMojo(amount))),
    ...(requestedTokens ?? [])
