...
import { Offering, Requesting } from '@shibgreen/icons';
...
import OfferBuilderXSHIBSection from './OfferBuilderXSHIBSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showXSHIB = !readOnly || !!xshib.length;
  ...
  const mutedXSHIB = nfts.length || tokens.length;
  const mutedTokens = xshib.length || nfts.length;
  const mutedNFTs = xshib.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showXSHIB && <OfferBuilderXSHIBSection name={`${name}.xshib`} offering={offering} muted={mutedXSHIB} />}
