...
import { FormatLargeNumber, mojoToCATLocaleString, mojoToSHIBgreenLocaleString, useLocale } from '@shibgreen/core';
import { useWallet } from '@shibgreen/wallets';
...
export default function OfferBuilderWalletBalance(props: OfferBuilderWalletBalanceProps) {
  ...
  const xshibBalance = useMemo(() => {
    if (isLoading || !wallet || !walletBalance || !('spendableBalance' in walletBalance)) {
      return undefined;
    }

    if (wallet.type === WalletType.STANDARD_WALLET) {
      return mojoToSHIBgreenLocaleString(walletBalance.spendableBalance, locale);
    }
  ...
  if (!isLoading && xshibBalance === undefined) {
    return null;
  }
