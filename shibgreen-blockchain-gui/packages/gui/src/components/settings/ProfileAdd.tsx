import { useCreateNewWalletMutation, useGetWalletBalanceQuery } from '@shibgreen/api-react';
import { ButtonLoading, shibgreenToMojo, EstimatedFee, Flex, Form, mojoToSHIBgreenLocaleString } from '@shibgreen/core';
...
export default function ProfileAdd() {
  ...
  function handleClick() {
    openExternal('https://faucet.shib.green/');
  }
  ...
  async function handleSubmit(data: CreateProfileData) {
    ...
    const walletId = await createProfile({
      ...
      options: { did_type: 'new', backup_dids: [], num_of_backup_ids_needed: '0', amount: 1, fee: shibgreenToMojo(fee) },
    }).unwrap();

    navigate(`/dashboard/settings/profiles/${walletId}`);
  }

  const standardBalance = mojoToSHIBgreenLocaleString(balance?.confirmedWalletBalance);
        ...
        <StyledCard>
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Balance: {standardBalance} XSHIB</Trans>
            </Typography>
          </Flex>
          <Flex flexDirection="column" gap={2.5} paddingBottom={1}>
            <EstimatedFee
          ...
          <Flex flexDirection="column" gap={2.5} paddingBottom={3}>
            <Typography variant="caption">
              <Trans>Recommended: 0.000005 XSHIB</Trans>
            </Typography>
          </Flex>
