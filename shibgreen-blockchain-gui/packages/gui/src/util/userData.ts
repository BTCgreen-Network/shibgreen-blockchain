...
export function getUserDataDir(): string {
  const shibgreenRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(shibgreenRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const shibgreenRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${shibgreenRootUserDataPath}`);
  app.setPath('userData', shibgreenRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(shibgreenRootUserDataPath, 'Local Storage', 'leveldb');
