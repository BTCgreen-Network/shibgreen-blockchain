...
Write-Output "   ---"
# download.shibgreennetwork.org is the CDN url behind all the files that are actually on pypi.shibgreennetwork.org/simple now
...
Write-Output "   ---"
Write-Output "Get SHIBGREEN_INSTALLER_VERSION"
# The environment variable SHIBGREEN_INSTALLER_VERSION needs to be defined
$env:SHIBGREEN_INSTALLER_VERSION = python .\build_scripts\installer-version.py -win
...
Write-Output "   ---"
Copy-Item ".\SHIBgreen-win32-x64" -Destination "$env:GITHUB_WORKSPACE\shibgreen-blockchain-gui\" -Recurse
Copy-Item ".\release-builds" -Destination "$env:GITHUB_WORKSPACE\shibgreen-blockchain-gui\" -Recurse
