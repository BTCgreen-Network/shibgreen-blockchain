...
# Builds CLI only rpm
CLI_RPM_BASE="shibgreen-blockchain-cli-$SHIBGREEN_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/shibgreen"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/shibgreen/"
ln -s ../../opt/shibgreen/shibgreen "dist/$CLI_RPM_BASE/usr/bin/shibgreen"
...
fpm -s dir -t rpm \
  ...
  --name shibgreen-blockchain-cli \
  --license Apache-2.0 \
  --version "$SHIBGREEN_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "SHIBgreen is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../shibgreen-blockchain-gui/packages/gui

# Change to the gui package
cd ../shibgreen-blockchain-gui/packages/gui || exit 1
...
PRODUCT_NAME="shibgreen"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="SHIBgreen Blockchain" \
  --config.rpm.packageName="shibgreen-blockchain"
...
GUI_RPM_NAME="shibgreen-blockchain-${SHIBGREEN_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${SHIBGREEN_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
