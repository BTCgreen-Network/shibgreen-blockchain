...
--appVersion=$SHIBGREEN_INSTALLER_VERSION --executable-name=shibgreen-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
  ...
  --arch "$REDHAT_PLATFORM" --options.version $SHIBGREEN_INSTALLER_VERSION \
  --license ../LICENSE --options.bin shibgreen-blockchain --options.name shibgreen-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
