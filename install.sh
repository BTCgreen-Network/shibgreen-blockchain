...
if [ "$(uname -m)" = "armv7l" ]; then
  ...
  echo "https://github.com/SHIBgreenNetwork/shibgreen-blockchain/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
# Get submodules
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with shibgreen
  ...
  # yum install python3 brings Python3.6 which is not supported by shibgreen
  cd ..
      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.11.* ]]; then
            echo "SHIBgreen requires Python version < 3.11.0" >&2
...
echo "https://keybase.io/team/shibgreen_network.public"
echo ""
echo "Try the Quick Start Guide to running shibgreen-blockchain:"
echo "https://github.com/SHIBgreenNetwork/shibgreen-blockchain/wiki/Quick-Start-Guide"
echo ""
