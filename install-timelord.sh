...
SHIBGREENVDF_VERSION=$(python -c 'import os; os.environ["SHIBGREEN_SKIP_SETUP"] = "1"; from setup import dependencies; t = [_ for _ in dependencies if _.startswith("chiavdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		...
		echo venv/bin/python -m pip install --force --no-binary chiavdf "$SHIBGREENVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary chiavdf "$SHIBGREENVDF_VERSION"
