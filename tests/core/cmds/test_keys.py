...
from shibgreen.cmds.shibgreen import cli
from shibgreen.cmds.keys import delete_all_cmd, generate_and_print_cmd, sign_cmd, verify_cmd
from shibgreen.util.config import load_config
from shibgreen.util.file_keyring import FileKeyring
from shibgreen.util.keychain import KeyData, DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from shibgreen.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct xshib_target_address entries.
        """
        ...
        # Verify that the config has the correct xshib_target_address entries
        address_matches = re.findall(r"xshib1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xshib_target_address"] == address
        assert config["pool"]["xshib_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original xshib_target_address entries.
        """
        ...
        assert existing_config["farmer"]["xshib_target_address"] == address
        assert existing_config["pool"]["xshib_target_address"] == address
        ...
        # Verify that the config's xshib_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xshib_target_address"] == existing_config["farmer"]["xshib_target_address"]
        assert config["pool"]["xshib_target_address"] == existing_config["pool"]["xshib_target_address"]
    ...
    def test_show_labels(self, empty_keyring, tmp_path):
        ...
        result = runner.invoke(cli, [*base_params, *cmd_params])
        assert result.output == "No keys are present in the keychain. Generate them with 'shibgreen keys generate'\n"
    ...
    def test_show(self, keyring_with_one_key, tmp_path):
        """
        Test that the `shibgreen keys show` command shows the correct key.
        """
    ...
    def test_show_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `shibgreen keys show --json` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key, tmp_path):
        """
        Test that the `shibgreen keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_show_mnemonic_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `shibgreen keys show --show-mnemonic-seed --json` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `shibgreen keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `shibgreen keys sign` command.
        """
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `shibgreen keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `shibgreen keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive search` command, searching a public and private key
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive search` command, searching for a wallet address
        """
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "xshib1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xshib1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive search` command, searching for a testnet wallet address
        """
        ...
        assert (
            result.output.find(
                (
                    "Found wallet address: "
                    "txshib1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7 (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `shibgreen keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive wallet-address` command, generating a couple of wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xshib1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    ...
                    "xshib1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
                )
            )
            != -1
        )
    ...
    def test_derive_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive wallet-address` command, generating a couple of testnet wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    "Wallet address 50 (m/12381n/8444n/2n/50n): "
                    "txshib1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfshfylvn"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    "Wallet address 51 (m/12381n/8444n/2n/51n): "
                    "txshib1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8s3utl6c"
                )
            )
            != -1
        )
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `shibgreen keys derive child-keys` command, generating a couple of derived keys.
        """
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `shibgreen keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `shibgreen keys migrate` command when a full migration is needed
        """
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from shibgreen.util import keyring_wrapper
