...
from shibgreen.cmds.wallet_funcs import print_offer_summary
...
@pytest.mark.asyncio
async def test_print_offer_summary_xshib(capsys: Any) -> None:
    summary_dict = {"xshib": 1_000_000_000_000}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "XSHIB (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xshib_and_cats(capsys: Any) -> None:
    summary_dict = {
        "xshib": 2_500_000_000_000,
    ...
    assert "XSHIB (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xshib_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "xshib": 0,
    ...
    assert "XSHIB (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xshib_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"xshib": 1}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "XSHIB (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
