...
from shibgreen.cmds.beta_funcs import default_beta_root_path
from shibgreen.cmds.shibgreen import cli
from shibgreen.util.beta_metrics import metrics_log_interval_default, metrics_log_interval_max, metrics_log_interval_min
...
def generate_example_submission_data(beta_root_path: Path, versions: int, logs: int) -> None:
    for version in range(versions):
        ...
        version_path.mkdir()
        shibgreen_blockchain_logs = version_path / "shibgreen-blockchain"
        plotting_logs = version_path / "plotting"
        shibgreen_blockchain_logs.mkdir()
        plotting_logs.mkdir()
        for i in range(logs):
            with open(shibgreen_blockchain_logs / f"beta_{i}.log", "w"):
                pass
            with open(shibgreen_blockchain_logs / f"beta_{i + 10}.gz", "w"):
                pass
...
def test_configure_no_beta_config(root_path_populated_with_config: Path) -> None:
    ...
    assert "beta test mode is not enabled, enable it first with `shibgreen beta enable`" in result.output
...
def test_prepare_submission(
    ...
    if exit_code == 0:
        ...
        with zipfile.ZipFile(submission_file) as zip_file:
            ...
            for version in range(versions):
                shibgreen_blockchain_logs = Path("shibgreen-blockchain")
                plotting_logs = Path("plotting")
                for i in range(logs):
                    assert shibgreen_blockchain_logs / f"beta_{i}.log" in all_files
                    assert shibgreen_blockchain_logs / f"beta_{i + 10}.gz" in all_files
