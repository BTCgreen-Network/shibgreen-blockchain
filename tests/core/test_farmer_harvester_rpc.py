...
from shibgreen.protocols.harvester_protocol import Plot
from shibgreen.rpc.farmer_rpc_api import (
    FilterItem,
...
from shibgreen.simulator.block_tools import get_plot_dir
from shibgreen.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from shibgreen.util.config import load_config, lock_and_load_config, save_config
from shibgreen.util.hash import std_hash
from shibgreen.util.ints import uint8, uint32, uint64
from shibgreen.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xshib"), encode_puzzle_hash(new_ph_2, "xshib"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xshib"), encode_puzzle_hash(observer_pool, "xshib")
    )
    ...
    assert config["farmer"]["xshib_target_address"] == encode_puzzle_hash(observer_farmer, "xshib")
    assert config["pool"]["xshib_target_address"] == encode_puzzle_hash(observer_pool, "xshib")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xshib")
