...
from shibgreen.data_layer.data_layer_util import NodeType, Status
from shibgreen.data_layer.data_store import DataStore
from shibgreen.types.blockchain_format.tree_hash import bytes32
...
@pytest.fixture(name="shibgreen_daemon", scope="function")
def shibgreen_daemon_fixture(shibgreen_root: SHIBgreenRoot) -> Iterator[None]:
    with closing_shibgreen_root_popen(shibgreen_root=shibgreen_root, args=[sys.executable, "-m", "shibgreen.daemon.server"]):
...
@pytest.fixture(name="shibgreen_data", scope="function")
def shibgreen_data_fixture(shibgreen_root: SHIBgreenRoot, shibgreen_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_shibgreen_root_popen(shibgreen_root=shibgreen_root, args=[os.fspath(scripts_path.joinpath("shibgreen_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
