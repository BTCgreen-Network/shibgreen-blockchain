...
from shibgreen.protocols.shared_protocol import protocol_version, capabilities
...
async def establish_connection(server: SHIBgreenServer, self_hostname: str, ssl_context) -> None:
...
class TestSSL:
    ...
    async def test_farmer(self, farmer_one_harvester, self_hostname):
        ...
        shibgreen_ca_crt_path, shibgreen_ca_key_path = shibgreen_ssl_ca_paths(bt.root_path, bt.config)
        ...
        generate_ca_signed_cert(shibgreen_ca_crt_path.read_bytes(), shibgreen_ca_key_path.read_bytes(), pub_crt, pub_key)
        ssl_context = ssl_context_for_client(shibgreen_ca_crt_path, shibgreen_ca_key_path, pub_crt, pub_key)
    ...
    async def test_full_node(self, wallet_node_sim_and_wallet, self_hostname):
        ...
        generate_ca_signed_cert(
            shibgreen_ca_crt_path.read_bytes(),
            shibgreen_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(shibgreen_ca_crt_path, shibgreen_ca_key_path, pub_crt, pub_key)
    ...
    async def test_introducer(self, introducer_service, self_hostname):
        ...
        shibgreen_ca_crt_path, shibgreen_ca_key_path = shibgreen_ssl_ca_paths(introducer_service.root_path, introducer_service.config)
    ...
    async def test_timelord(self, timelord_service, self_hostname):
        ...
        shibgreen_ca_crt_path, shibgreen_ca_key_path = shibgreen_ssl_ca_paths(timelord_service.root_path, timelord_service.config)
