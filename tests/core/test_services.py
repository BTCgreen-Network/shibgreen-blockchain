...
from shibgreen.rpc.data_layer_rpc_client import DataLayerRpcClient
...
from shibgreen.util.config import lock_and_load_config, save_config
from shibgreen.util.ints import uint16
from shibgreen.util.misc import sendable_termination_signals
from tests.core.data_layer.util import SHIBgreenRoot
from tests.util.misc import closing_shibgreen_root_popen
...
@pytest.mark.asyncio
async def test_daemon_terminates(signal_number: signal.Signals, shibgreen_root: SHIBgreenRoot) -> None:
    port = find_available_listen_port()
    with lock_and_load_config(root_path=shibgreen_root.path, filename="config.yaml") as config:
        config["daemon_port"] = port
        save_config(root_path=shibgreen_root.path, filename="config.yaml", config_data=config)

    with closing_shibgreen_root_popen(shibgreen_root=shibgreen_root, args=[sys.executable, "-m", "shibgreen.daemon.server"]) as process:
        client = await wait_for_daemon_connection(root_path=shibgreen_root.path, config=config)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        [DataLayerRpcClient.create, "shibgreen.server.start_data_layer", "data_layer"],
        [FarmerRpcClient.create, "shibgreen.server.start_farmer", "farmer"],
        [FullNodeRpcClient.create, "shibgreen.server.start_full_node", "full_node"],
        [HarvesterRpcClient.create, "shibgreen.server.start_harvester", "harvester"],
        [WalletRpcClient.create, "shibgreen.server.start_wallet", "wallet"],
        # TODO: review and somehow test the other services too
        # [, "shibgreen.server.start_introducer", "introducer"],
        # [, "shibgreen.seeder.start_crawler", ""],
        # [, "shibgreen.server.start_timelord", "timelord"],
        # [, "shibgreen.timelord.timelord_launcher", ],
        # [, "shibgreen.simulator.start_simulator", ],
        # [, "shibgreen.data_layer.data_layer_server", "data_layer"],
    ],
)
@pytest.mark.asyncio
async def test_services_terminate(
    signal_number: signal.Signals,
    shibgreen_root: SHIBgreenRoot,
    ...
    with closing_shibgreen_root_popen(
        shibgreen_root=shibgreen_root,
        args=[sys.executable, "-m", "shibgreen.daemon.server"],
    ):
        ...
        # farmer.
        daemon_client = await wait_for_daemon_connection(root_path=shibgreen_root.path, config=config)
