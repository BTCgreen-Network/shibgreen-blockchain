...
from shibgreen.clvm.spend_sim import SimClient, SpendSim
...
from shibgreen.util.config import create_default_shibgreen_config, lock_and_load_config
from shibgreen.util.ints import uint16
from shibgreen.simulator.setup_services import setup_daemon, setup_introducer, setup_timelord
from shibgreen.util.task_timing import (
...
from shibgreen.simulator.setup_nodes import (
...
from shibgreen.simulator.wallet_tools import WalletTool
...
from shibgreen.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
from shibgreen.simulator.keyring import TempKeyring
from shibgreen.simulator.setup_nodes import setup_farmer_multi_harvester
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the shibgreen root directory
...
async def empty_blockchain(request):
    ...
    from shibgreen.simulator.setup_nodes import test_constants
...
async def wallet_node_sim_and_wallet() -> AsyncIterator[
    Tuple[List[Union[FullNodeAPI, FullNodeSimulator]], List[Tuple[Wallet, SHIBgreenServer]], BlockTools],
]:
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, xshib_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, xshib_spam_amount):
        yield _
...
@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], SHIBgreenServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def tmp_shibgreen_root(tmp_path):
    """
    Create a temp directory and populate it with an empty shibgreen_root directory.
    """
    path: Path = tmp_path / "shibgreen_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_shibgreen_root) -> Path:
    """
    Create a temp shibgreen_root directory and populate it with a default config.yaml.
    Returns the shibgreen_root path.
    """
    root_path: Path = tmp_shibgreen_root
...
@pytest.fixture(name="shibgreen_root", scope="function")
def shibgreen_root_fixture(tmp_path: Path, scripts_path: Path) -> SHIBgreenRoot:
    root = SHIBgreenRoot(path=tmp_path.joinpath("shibgreen_root"), scripts_path=scripts_path)
    root.run(args=["init"])
