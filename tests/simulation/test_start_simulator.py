...
from shibgreen.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
from shibgreen.simulator.simulator_test_tools import get_full_shibgreen_simulator, get_puzzle_hash_from_key
...
class TestStartSimulator:
    ...
    async def get_shibgreen_simulator(
        self, tmp_path: Path, empty_keyring: Keychain
    ) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain], None]:
        async for simulator_args in get_full_shibgreen_simulator(shibgreen_root=tmp_path, keychain=empty_keyring):
            yield simulator_args

    @pytest.mark.asyncio
    async def test_start_simulator(
        self, get_shibgreen_simulator: Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain]
    ) -> None:
        simulator, root_path, config, mnemonic, fingerprint, keychain = get_shibgreen_simulator
