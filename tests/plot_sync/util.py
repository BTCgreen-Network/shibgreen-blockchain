...
@dataclass
class WSSHIBgreenConnectionDummy:
...
def get_dummy_connection(node_type: NodeType, peer_id: Optional[bytes32] = None) -> WSSHIBgreenConnectionDummy:
    return WSSHIBgreenConnectionDummy(node_type, bytes32(token_bytes(32)) if peer_id is None else peer_id)
