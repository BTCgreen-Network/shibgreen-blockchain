...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            shibgreen_announcements: List[Announcement] = Offer.calculate_announcements(shibgreen_requested_payments)
            shibgreen_secured_bundle: SpendBundle = self.generate_secure_bundle(shibgreen_coins, shibgreen_announcements, 1000)
            shibgreen_offer = Offer(shibgreen_requested_payments, shibgreen_secured_bundle)
            ...
            new_offer = Offer.aggregate([shibgreen_offer, red_offer])
