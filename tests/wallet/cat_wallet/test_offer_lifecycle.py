...
from shibgreen.wallet.cat_wallet.cat_utils import (
...
from shibgreen.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XSHIB coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            shibgreen_coins: List[Coin] = all_coins[None]
            ...
            # Create an XSHIB Offer for RED
            shibgreen_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            shibgreen_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                shibgreen_requested_payments, shibgreen_coins
            )
            shibgreen_announcements: List[Announcement] = Offer.calculate_announcements(shibgreen_requested_payments, driver_dict)
            shibgreen_secured_bundle: SpendBundle = generate_secure_bundle(shibgreen_coins, shibgreen_announcements, 1000)
            shibgreen_offer = Offer(shibgreen_requested_payments, shibgreen_secured_bundle, driver_dict)
            assert not shibgreen_offer.is_valid()

            # Create a RED Offer for XSHIB
            ...
            new_offer = Offer.aggregate([shibgreen_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XSHIB and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xshib": 900, str_to_tail_hash("red").hex(): 350},
