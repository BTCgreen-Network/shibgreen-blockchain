...
class TestCATTrades:
    ...
    async def test_cat_trades(self, wallets_prefarm):
        ...
        MAKER_SHIBGREEN_BALANCE = 20 * 1000000000000 - 100
        TAKER_SHIBGREEN_BALANCE = 20 * 1000000000000 - 100
        ...
        assert len(taker_txs) == 3  # One for each: the outgoing CAT, the incoming shibgreen, and the outgoing shibgreen fee
        ...
        assert len(maker_txs) == 1  # The other side will show up as a regular incoming transaction
        assert len(taker_txs) == 2  # One for each: the outgoing shibgreen, the incoming CAT
        ...
        # shibgreen_for_multiple_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(shibgreen_for_multiple_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        MAKER_SHIBGREEN_BALANCE = 20 * 1000000000000 - 100
        MAKER_CAT_BALANCE = 100
        TAKER_SHIBGREEN_BALANCE = 20 * 1000000000000
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_SHIBGREEN_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_SHIBGREEN_BALANCE - FEE)
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, MAKER_CAT_BALANCE)
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_SHIBGREEN_BALANCE)
