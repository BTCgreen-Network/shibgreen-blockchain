...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted):
        ...
        try:
            ...
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "xshib")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "xshib"))
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "xshib"))  # Create a pending tx
            ...
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_by_addr, "xshib"))
            await client.farm_block(encode_puzzle_hash(ph_by_addr, "xshib"))
            ...
            tx_for_address = await wallet_rpc_api.get_transactions(
                {"wallet_id": "1", "to_address": encode_puzzle_hash(ph_by_addr, "xshib")}
            )
            ...
            assert summary == {"offered": {"xshib": 5}, "requested": {col.hex(): 1}, "fees": 1}
