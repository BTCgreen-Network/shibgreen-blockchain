...
from shibgreen.protocols.shared_protocol import Capability
from shibgreen.protocols.wallet_protocol import RequestAdditions, RespondAdditions, RespondBlockHeaders, SendTransaction
from shibgreen.server.outbound_message import Message, make_msg
from shibgreen.simulator.block_tools import test_constants
from shibgreen.simulator.simulator_protocol import FarmNewBlockProtocol
from shibgreen.simulator.time_out_assert import time_out_assert, time_out_assert_not_none
...
from shibgreen.wallet.util.wallet_sync_utils import PeerRequestException
from shibgreen.wallet.util.wallet_types import AmountWithPuzzlehash
from shibgreen.wallet.wallet_coin_record import WalletCoinRecord
from shibgreen.wallet.wallet_weight_proof_handler import get_wp_fork_point
...
class TestWalletSync:
    ...
    6. Clear all coins from the dust wallet.
       Send to the dust wallet "spam_filter_after_n_txs" coins that are equal in value to "xshib_spam_amount".
       Send 1 mojo from the dust wallet. The dust wallet should receive a change coin valued at "xshib_spam_amount-1".
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "spam_filter_after_n_txs, xshib_spam_amount, dust_value",
        [
    ...
    async def test_dusted_wallet(
        ...
        # It's also possible to obtain the current settings for spam_filter_after_n_txs and xshib_spam_amount
        # spam_filter_after_n_txs = wallets[0][0].config["spam_filter_after_n_txs"]
        # xshib_spam_amount = wallets[0][0].config["xshib_spam_amount"]
        # dust_value=1
        ...
        # Current max value for xshib_spam_amount is 0.01 XSHIB.
        ...
        assert xshib_spam_amount >= 1
        assert xshib_spam_amount <= 10000000000
        assert dust_value >= 1
        ...
        # The dust is only filtered at this point if spam_filter_after_n_txs is 0 and xshib_spam_amount is > dust_value.
        if spam_filter_after_n_txs > 0:
            dust_coins = 1
            large_dust_coins = 0
            large_dust_balance = 0
        elif xshib_spam_amount <= dust_value:
            dust_coins = 0
            large_dust_coins = 1
        ...
        small_unspent_count = len([r for r in all_unspent if r.coin.amount < xshib_spam_amount])
        ...
        log.info(f"xshib_spam_amount {xshib_spam_amount}")
        ...
        for i in range(large_coins):
            payee_ph = await dust_wallet.get_new_puzzlehash()
            payees.append({"amount": uint64(xshib_spam_amount), "puzzlehash": payee_ph, "memos": []})
        ...
        large_coin_balance = large_coins * xshib_spam_amount

        # Determine whether the filter should have been activated.
        # Make sure the number of coins matches the expected number.
        # At this point, nothing should be getting filtered unless spam_filter_after_n_txs is 0.
        assert dust_coins == spam_filter_after_n_txs
        assert balance == dust_coins * dust_value + large_coins * xshib_spam_amount + large_dust_balance
        ...
        if dust_value >= xshib_spam_amount:
            large_dust_coins += 1
            large_dust_balance += dust_value
        ...
        for i in range(5):
            ...
            payees.append({"amount": uint64(xshib_spam_amount + i), "puzzlehash": payee_ph, "memos": []})
            large_coins += 1
            large_coin_balance += xshib_spam_amount + i

            payee_ph = await dust_wallet.get_new_puzzlehash()

            # Make sure we are always creating coins with a positive value.
            if xshib_spam_amount - dust_value - i > 0:
                payees.append({"amount": uint64(xshib_spam_amount - dust_value - i), "puzzlehash": payee_ph, "memos": []})
            else:
                payees.append({"amount": uint64(dust_value), "puzzlehash": payee_ph, "memos": []})
            # In cases where xshib_spam_amount is sufficiently low,
            # the new dust should be considered a large coina and not be filtered.
            if xshib_spam_amount <= dust_value:
                large_dust_coins += 1
                large_dust_balance += dust_value
        ...
        # The size of the coin to send the dust wallet is the same as xshib_spam_amount
        if xshib_spam_amount > 1:
            coin_value = xshib_spam_amount
        else:
        ...
        metadata = Program.to(
            [
                ("u", ["https://www.shib.green/img/branding/shibgreen-logo.svg"]),
