...
from shibgreen.wallet.wallet_node import WalletNode, get_wallet_db_path
from shibgreen.wallet.wallet_state_manager import WalletStateManager
from shibgreen.simulator.block_tools import BlockTools
...
class TestWalletSimulator:
    ...
    async def test_wallet_coinbase(
        self,
        wallet_node_sim_and_wallet: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SHIBgreenServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction(
        self,
        two_wallet_nodes: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SHIBgreenServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 5
    ...
    async def test_wallet_send_to_three_peers(
        self,
        three_sim_two_wallets: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SHIBgreenServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_wallet_make_transaction_hop(
        self,
        two_wallet_nodes_five_freeze: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SHIBgreenServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    #         outbound: List[WSSHIBgreenConnection] = wallet.server.get_outgoing_connections()
    ...
    async def test_address_sliding_window(
        self,
        wallet_node_100_pk: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SHIBgreenServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
    ...
    async def test_sign_message(
        ...
        await server_2.start_client(PeerInfo(self_hostname, uint16(server_1._port)), None)
        message = "Hello World"
        response = await api_0.sign_message_by_address({"address": encode_puzzle_hash(ph, "xshib"), "message": message})
...
def test_get_wallet_db_path_v2_r1() -> None:
    root_path: Path = Path("/x/y/z/.shibgreen/mainnet").resolve()
    config: Dict[str, Any] = {
...
def test_get_wallet_db_path_testnet() -> None:
    root_path: Path = Path("/x/y/z/.shibgreen/testnet").resolve()
    config: Dict[str, Any] = {
        "database_path": "wallet/db/blockchain_wallet_v2_CHALLENGE_KEY.sqlite",
