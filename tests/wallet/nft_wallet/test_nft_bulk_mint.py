...
async def test_nft_mint_from_did(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected_xshib_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_xshib_bal)
...
async def test_nft_mint_from_did_rpc(two_wallet_nodes: Any, trusted: Any, self_hostname: str) -> None:
    ...
    try:
        ...
        target_list = [encode_puzzle_hash((ph_taker), "xshib") for x in range(n)]
        royalty_address = encode_puzzle_hash(bytes32(token_bytes(32)), "xshib")
        royalty_percentage = 300
        fee = 100
        required_amount = n + (fee * n)
        xshib_coins = await client.select_coins(amount=required_amount, wallet_id=wallet_maker.id())
        funding_coin = xshib_coins[0]
        assert funding_coin.amount >= required_amount
        funding_coin_dict = xshib_coins[0].to_json_dict()
        chunk = 5
        ...
        for i in range(0, n, chunk):
            resp: Dict[str, Any] = await client.nft_mint_bulk(
                ...
                mint_total=n,
                xshib_coins=[next_coin.to_json_dict()],
                xshib_change_target=funding_coin_dict["puzzle_hash"],
            ...
            spends.append(sb)
            xshib_adds = [c for c in sb.additions() if c.puzzle_hash == funding_coin.puzzle_hash]
            assert len(xshib_adds) == 1
            next_coin = xshib_adds[0]
...
@pytest.mark.asyncio
async def test_nft_mint_from_did_multiple_xshib(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    # Grab two coins for testing that we can create a bulk minting with more than 1 xshib coin
    xshib_coins_1 = await wallet_maker.select_coins(amount=10000)
    xshib_coins_2 = await wallet_maker.select_coins(amount=10000, exclude=xshib_coins_1)
    xshib_coins = xshib_coins_1.union(xshib_coins_2)
    ...
    # confirm that the spend uses the right amount of xshib
    expected_xshib_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_maker.get_confirmed_balance, expected_xshib_bal)
...
@pytest.mark.asyncio
async def test_nft_mint_from_xshib(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    sb = await nft_wallet_maker.mint_from_xshib(
        metadata_list, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
...
@pytest.mark.asyncio
async def test_nft_mint_from_xshib_rpc(two_wallet_nodes: Any, trusted: Any, self_hostname: str) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    bt = full_node_api.bt
    full_node_server = full_node_api.server
...
@pytest.mark.asyncio
async def test_nft_mint_from_xshib_multiple_xshib(two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
