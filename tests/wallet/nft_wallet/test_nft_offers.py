...
async def test_nft_offer_with_fee(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 xshib
    ...
    xshib_request = 100
    maker_fee = uint64(10)
    offer_nft_for_xshib = {wallet_maker.id(): xshib_request, nft_asset_id: -1}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_xshib, driver_dict, fee=maker_fee
    )
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + xshib_request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - xshib_request - taker_fee)
    ...
    # MAKE SECOND TRADE: 100 xshib for 1 NFT
    ...
    xshib_offered = 1000
    maker_fee = uint64(10)
    offer_xshib_for_nft = {wallet_maker.id(): -xshib_offered, nft_to_buy_asset_id: 1}
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_xshib_for_nft, driver_dict_to_buy, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
    ...
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - xshib_offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + xshib_offered - taker_fee)
...
async def test_nft_offer_nft_for_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.shib.green/image2.html"]),
