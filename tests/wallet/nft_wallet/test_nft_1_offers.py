...
from shibgreen.simulator.full_node_simulator import FullNodeSimulator
...
from shibgreen.wallet.outer_puzzles import create_asset_id, match_puzzle
...
async def test_nft_offer_sell_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    # maker create offer: NFT for xshib
    ...
    xshib_requested = 1000
    maker_fee = uint64(433)

    offer_did_nft_for_xshib = {nft_to_offer_asset_id: -1, wallet_maker.id(): xshib_requested}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_did_nft_for_xshib, {}, fee=maker_fee
    )
    ...
    expected_royalty = uint64(xshib_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xshib_requested + expected_royalty
    expected_taker_balance = funds - taker_fee - xshib_requested - expected_royalty
...
async def test_nft_offer_request_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    offer_dict = {nft_to_request_asset_id: 1, wallet_maker.id(): -xshib_offered}
    ...
    # assert payments and royalties
    expected_royalty = uint64(xshib_offered * royalty_basis_pts / 10000)
    expected_maker_balance = funds - maker_fee - xshib_offered - expected_royalty
    expected_taker_balance = funds - 2 - taker_fee + xshib_offered + expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_did_to_did(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected_royalty = uint64(xshib_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xshib_requested + expected_royalty
    expected_taker_balance = funds - 1 - taker_fee - xshib_requested - expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_nft_for_cat(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    offer_did_nft_for_xshib = {nft_to_offer_asset_id: -1, cat_wallet_maker.id(): cats_requested}
...
async def test_complex_nft_offer(two_wallet_nodes: Any, trusted: Any) -> None:
    """
    This test is going to create an offer where the maker offers 1 NFT and 1 CAT for 2 NFTs, an XSHIB and a CAT
    """
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    ...
    maker_xshib_royalties_expected = maker_royalty_summary[nft_to_offer_asset_id_maker][0]["amount"]
    ...
    funds_maker = int(funds_maker - FEE + XSHIB_REQUESTED + maker_xshib_royalties_expected)
    funds_taker = int(funds_taker - FEE - XSHIB_REQUESTED - maker_xshib_royalties_expected)
    ...
    # Now let's make sure the final wallet state is correct
    funds_maker = int(funds_maker + XSHIB_REQUESTED / 2)
    funds_taker = int(funds_taker - XSHIB_REQUESTED / 2)
