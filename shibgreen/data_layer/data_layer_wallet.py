...
from shibgreen.data_layer.data_layer_errors import OfferIntegrityError
from shibgreen.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from shibgreen.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from shibgreen.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from shibgreen.wallet.sign_coin_spends import sign_coin_spends
from shibgreen.wallet.trading.offer import NotarizedPayment, Offer
...
from shibgreen.wallet.util.merkle_utils import _simplify_merkle_proof
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSSHIBgreenConnection) -> CoinState:
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            shibgreen_tx = await self.create_tandem_xshib_tx(
                fee, Announcement(current_coin.name(), b"$"), coin_announcement=True
            )
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, shibgreen_tx.spend_bundle])
            ...
            txs: List[TransactionRecord] = [dl_tx, shibgreen_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSSHIBgreenConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            shibgreen_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, shibgreen_tx.spend_bundle])
                ),
                dataclasses.replace(shibgreen_tx, spend_bundle=None),
            ]
