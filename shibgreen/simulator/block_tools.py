...
from shibgreen.cmds.init_funcs import create_default_shibgreen_config
...
from shibgreen.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
...
from shibgreen.plotting.create_plots import PlotKeys, create_plots
...
from shibgreen.simulator.time_out_assert import time_out_assert_custom_interval
...
from shibgreen.util.config import config_path_for_filename, load_config, lock_config, override_config, save_config
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, self.log, user="testing-1.8.0", service="shibgreen-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `shibgreen keys generate`")
