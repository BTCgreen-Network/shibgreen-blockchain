...
from shibgreen.daemon.server import WebSocketServer, daemon_launch_lock_path
from shibgreen.simulator.full_node_simulator import FullNodeSimulator
from shibgreen.simulator.socket import find_available_listen_port
from shibgreen.simulator.ssl_certs import (
...
from shibgreen.simulator.start_simulator import async_main as start_simulator_main
from shibgreen.ssl.create_ssl import create_all_ssl
from shibgreen.types.blockchain_format.sized_bytes import bytes32
from shibgreen.util.bech32m import encode_puzzle_hash
from shibgreen.util.config import create_default_shibgreen_config, load_config, save_config
from shibgreen.util.errors import KeychainFingerprintExists
from shibgreen.util.ints import uint32
from shibgreen.util.keychain import Keychain
from shibgreen.util.lock import Lockfile
...
def create_config(
    ...
    # create shibgreen directories
    create_default_shibgreen_config(shibgreen_root)
    create_all_ssl(
        shibgreen_root,
    ...
    # load config
    config = load_config(shibgreen_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txshib")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(shibgreen_root, "config.yaml", config)
    return config


async def start_simulator(shibgreen_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=shibgreen_root)
    await service.start()
...
async def get_full_shibgreen_simulator(
    shibgreen_root: Path,
    ...
    """
    A shibgreen root Path is required.
    The shibgreen root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the shibgreen test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(shibgreen_root)):
        ...
        crt_path = shibgreen_root / config["daemon_ssl"]["private_crt"]
        key_path = shibgreen_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = shibgreen_root / config["private_ssl_ca"]["crt"]
        ca_key_path = shibgreen_root / config["private_ssl_ca"]["key"]

        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(shibgreen_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()

        async for simulator in start_simulator(shibgreen_root, automated_testing):
            yield simulator, shibgreen_root, config, mnemonic, fingerprint, keychain
