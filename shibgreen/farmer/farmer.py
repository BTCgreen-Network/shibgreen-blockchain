...
from shibgreen.plot_sync.delta import Delta
from shibgreen.plot_sync.receiver import Receiver
from shibgreen.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from shibgreen.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from shibgreen.util.errors import KeychainProxyConnectionFailure
...
from shibgreen.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'shibgreen keys generate' or open the UI."
        try:
        ...
        if "xshib_target_address" not in self.config:
            self.config = config["farmer"]
        if "xshib_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xshib_target_address" not in self.config or "xshib_target_address" not in self.pool_config:
            log.debug("xshib_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["xshib_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["xshib_target_address"]
    ...
    def on_disconnect(self, connection: WSSHIBgreenConnection):
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["xshib_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["xshib_target_address"] = pool_target_encoded
