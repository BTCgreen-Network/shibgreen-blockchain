"""
NOTE: This contains duplicate code from `shibgreen.cmds.plots`.
After `shibgreen plots create` becomes obsolete, consider removing it from there.
"""
import asyncio
import logging
...
from shibgreen.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def get_shibgreenpos_install_info() -> Optional[Dict[str, Any]]:
    shibgreenpos_version: str = pkg_resources.get_distribution("chiapos").version
    return {"display_name": "SHIBgreen Proof of Space", "version": shibgreenpos_version, "installed": True}
...
def plot_shibgreen(args, root_path):
    try:
