...
from shibgreen.plotters.shibgreenpos import get_shibgreenpos_install_info, plot_shibgreen
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        if option is Options.NUM_BUCKETS:
            u_default = 0 if name == "shibgreenpos" else 256
            parser.add_argument(
                "-u",
                "--buckets",
                type=int,
...
def call_plotters(root_path: Path, args):
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SHIBGREEN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("shibgreen plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, shibgreen_plotter_options, "shibgreenpos", "Create a plot with the default shibgreen plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'shibgreen plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        plotters.print_help()
    elif args.plotter == "shibgreenpos":
        ...
        show_plotters_version(shibgreen_root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    shibgreenpos: Optional[Dict[str, Any]] = get_shibgreenpos_install_info()
    ...
    if shibgreenpos is not None:
        plotters["shibgreenpos"] = shibgreenpos
...
def show_plotters_version(root_path: Path):
    ...
    if "shibgreenpos" in info and "version" in info["shibgreenpos"]:
        print(f"shibgreenpos: {info['shibgreenpos']['version']}")
