...
from shibgreen.plotting.create_plots import resolve_plot_keys
from shibgreen.plotters.plotters_util import (
    run_plotter,
    run_command,
...
def get_madmax_exec_venv_path(ksize: int = 32) -> Optional[Path]:
    ...
    if not venv_bin_path:
        return None
    madmax_exec = "shibgreen_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the shibgreen_plot_k34 executable for k-sizes > 32
...
def get_madmax_exec_package_path(ksize: int = 32) -> Path:
    ...
    madmax_exec: str = "shibgreen_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the shibgreen_plot_k34 executable for k-sizes > 32
    if sys.platform in ["win32", "cygwin"]:
        madmax_exec += ".exe"
...
def plot_madmax(args, shibgreen_root_path: Path, plotters_root_path: Path):
    ...
    try:
        asyncio.run(run_plotter(shibgreen_root_path, args.plotter, call_args, progress))
    except Exception as e:
