...
from shibgreen.util.ints import uint64, uint32, uint8, uint128
from shibgreen.wallet.did_wallet.did_wallet_puzzles import create_fullpuz
...
from shibgreen.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from shibgreen.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSSHIBgreenConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSSHIBgreenConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            shibgreen_tx = await self.create_tandem_xshib_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if shibgreen_tx is not None and shibgreen_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(shibgreen_tx)
