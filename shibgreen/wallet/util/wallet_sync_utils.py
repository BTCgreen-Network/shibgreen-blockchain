...
from shibgreen.types.blockchain_format.coin import hash_coin_ids, Coin
...
from shibgreen.util.merkle_set import confirm_not_included_already_hashed, confirm_included_already_hashed, MerkleSet
from shibgreen.wallet.util.peer_request_cache import PeerRequestCache
...
async def fetch_last_tx_from_peer(height: uint32, peer: WSSHIBgreenConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSSHIBgreenConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSSHIBgreenConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSSHIBgreenConnection, bool]],
