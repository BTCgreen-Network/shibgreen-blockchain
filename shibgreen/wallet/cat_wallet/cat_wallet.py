...
from shibgreen.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
                ...
                if need_shibgreen_transaction:
                    if fee > regular_shibgreen_to_claim:
                        shibgreen_tx, _ = await self.create_tandem_xshib_tx(
                            fee, uint64(regular_shibgreen_to_claim), announcement_to_assert=announcement
                        )
                        ...
                        shibgreen_tx, _ = await self.create_tandem_xshib_tx(fee, uint64(regular_shibgreen_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
