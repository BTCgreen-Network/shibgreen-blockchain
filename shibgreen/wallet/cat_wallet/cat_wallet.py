...
class CATWallet:
    ...
    async def create_tandem_xshib_tx(
        ...
        if fee > amount_to_claim:
            shibgreen_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_shibgreen_transaction:
                    if fee > regular_shibgreen_to_claim:
                        ...
                        shibgreen_tx, _ = await self.create_tandem_xshib_tx(
                            fee, uint64(regular_shibgreen_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
