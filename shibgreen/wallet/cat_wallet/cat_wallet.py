...
from shibgreen.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from shibgreen.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            shibgreen_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        shibgreen_tx = dataclasses.replace(shibgreen_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(shibgreen_tx)
    ...
    async def create_tandem_xshib_tx(
        ...
        if fee > amount_to_claim:
            shibgreen_coins = await self.standard_wallet.select_coins(
                fee,
                min_coin_amount=min_coin_amount,
                max_coin_amount=max_coin_amount,
                excluded_coin_amounts=exclude_coin_amounts,
            )
            origin_id = list(shibgreen_coins)[0].name()
            ...
            message = None
            for spend in shibgreen_tx.spend_bundle.coin_spends:
            ...
            selected_amount = sum([c.amount for c in shibgreen_coins])
            shibgreen_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return shibgreen_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XSHIB as part of this
        regular_shibgreen_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_shibgreen_to_claim = payment_amount

        need_shibgreen_transaction = (fee > 0 or regular_shibgreen_to_claim > 0) and (fee - regular_shibgreen_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_shibgreen_transaction:
                    if fee > regular_shibgreen_to_claim:
                        shibgreen_tx, _ = await self.create_tandem_xshib_tx(
                            fee,
                            uint64(regular_shibgreen_to_claim),
                    ...
                    elif regular_shibgreen_to_claim > fee:
                        shibgreen_tx, _ = await self.create_tandem_xshib_tx(
                            fee,
                            uint64(regular_shibgreen_to_claim),
                            min_coin_amount=min_coin_amount,
        ...
        shibgreen_spend_bundle = SpendBundle([], G2Element())
        if shibgreen_tx is not None and shibgreen_tx.spend_bundle is not None:
            shibgreen_spend_bundle = shibgreen_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    shibgreen_spend_bundle,
                ]
            ),
            shibgreen_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        if shibgreen_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=shibgreen_tx.confirmed_at_height,
                    created_at_time=shibgreen_tx.created_at_time,
                    to_puzzle_hash=shibgreen_tx.to_puzzle_hash,
                    amount=shibgreen_tx.amount,
                    fee_amount=shibgreen_tx.fee_amount,
                    confirmed=shibgreen_tx.confirmed,
                    sent=shibgreen_tx.sent,
                    spend_bundle=None,
                    additions=shibgreen_tx.additions,
                    removals=shibgreen_tx.removals,
                    wallet_id=shibgreen_tx.wallet_id,
                    sent_to=shibgreen_tx.sent_to,
                    trade_id=shibgreen_tx.trade_id,
                    type=shibgreen_tx.type,
                    name=shibgreen_tx.name,
                    memos=[],
                )
            )
