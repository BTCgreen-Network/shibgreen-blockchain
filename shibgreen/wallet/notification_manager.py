...
from shibgreen.wallet.notification_store import Notification, NotificationStore
from shibgreen.wallet.transaction_record import TransactionRecord
from shibgreen.wallet.util.compute_memos import compute_memos_for_spend
from shibgreen.wallet.util.notifications import construct_notification
...
class NotificationManager:
    ...
    async def send_new_notification(
        ...
        shibgreen_tx = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
            amount,
            notification_hash,
            fee,
            coins=coins,
        ...
        full_tx: TransactionRecord = dataclasses.replace(
            shibgreen_tx, spend_bundle=SpendBundle.aggregate([shibgreen_tx.spend_bundle, extra_spend_bundle])
        )
        return full_tx
