...
from shibgreen.types.weight_proof import WeightProof, SubEpochData
from shibgreen.util.byte_types import hexstr_to_bytes
from shibgreen.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from shibgreen.util.default_root import STANDALONE_ROOT_PATH
from shibgreen.util.ints import uint32, uint64
from shibgreen.util.keychain import KeyringIsLocked, Keychain
from shibgreen.util.path import mkdir, path_from_root
from shibgreen.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
...
class WalletNode:
    ...
    server: Optional[SHIBgreenServer]
    log: logging.Logger
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.shibgreen.com"]),
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSSHIBgreenConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_puzzle_solution(self, peer: WSSHIBgreenConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSSHIBgreenConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSSHIBgreenConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
