...
from shibgreen.daemon.keychain_proxy import (
    KeychainProxy,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from shibgreen.protocols import wallet_protocol
from shibgreen.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from shibgreen.protocols.protocol_message_types import ProtocolMessageTypes
from shibgreen.protocols.wallet_protocol import (
    CoinState,
    RespondBlockHeader,
    RespondToCoinUpdates,
    RespondToPhUpdates,
)
from shibgreen.rpc.rpc_server import default_get_connections
from shibgreen.server.node_discovery import WalletPeers
...
from shibgreen.util.chunks import chunks
from shibgreen.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from shibgreen.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from shibgreen.util.profiler import mem_profile_task, profile_task
from shibgreen.wallet.transaction_record import TransactionRecord
from shibgreen.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from shibgreen.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from shibgreen.wallet.util.wallet_sync_utils import (
...
from shibgreen.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    _server: Optional[SHIBgreenServer] = None
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'shibgreen keys' program.")
            return None
    ...
    def set_server(self, server: SHIBgreenServer):
        self._server = server
        self.initialize_wallet_peers()
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.shib.green"]),
    ...
    def on_disconnect(self, peer: WSSHIBgreenConnection):
    ...
    async def on_connect(self, peer: WSSHIBgreenConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def long_sync(
        ...
        full_node: WSSHIBgreenConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSSHIBgreenConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSSHIBgreenConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSSHIBgreenConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSSHIBgreenConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSSHIBgreenConnection]:
        """
        ...
        synced_and_trusted: List[WSSHIBgreenConnection] = []
        synced: List[WSSHIBgreenConnection] = []
        trusted: List[WSSHIBgreenConnection] = []
        neither: List[WSSHIBgreenConnection] = []
        all_nodes: List[WSSHIBgreenConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSSHIBgreenConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSSHIBgreenConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSSHIBgreenConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSSHIBgreenConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSSHIBgreenConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSSHIBgreenConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSSHIBgreenConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSSHIBgreenConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSSHIBgreenConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSSHIBgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSSHIBgreenConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
