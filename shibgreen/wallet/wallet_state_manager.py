...
from shibgreen.consensus.coinbase import farmer_parent_id, pool_parent_id
...
from shibgreen.data_layer.dl_wallet_store import DataLayerStore
...
from shibgreen.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from shibgreen.wallet.cat_wallet.cat_wallet import CATWallet
from shibgreen.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from shibgreen.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from shibgreen.wallet.key_val_store import KeyValStore
from shibgreen.wallet.nft_wallet.nft_info import NFTWalletInfo
from shibgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
...
from shibgreen.wallet.notification_manager import NotificationManager
from shibgreen.wallet.outer_puzzles import AssetType
from shibgreen.wallet.puzzle_drivers import PuzzleInfo
from shibgreen.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
...
from shibgreen.wallet.util.address_type import AddressType
from shibgreen.wallet.util.compute_hints import compute_coin_hints
from shibgreen.wallet.util.transaction_type import TransactionType
from shibgreen.wallet.util.wallet_sync_utils import last_change_height_cs, PeerRequestException
...
from shibgreen.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSSHIBgreenConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xshib_spam_amount = self.config.get("xshib_spam_amount", 1000000)
        ...
        small_unspent_count = await self.coin_store.count_small_unspent(xshib_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xshib_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSSHIBgreenConnection) -> Optional[bytes32]:
        # Get minter DID
