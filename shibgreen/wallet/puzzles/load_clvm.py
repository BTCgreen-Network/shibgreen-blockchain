...
recompile_requested = (os.environ.get("SHIBGREEN_DEV_COMPILE_CLVM_ON_IMPORT", "") != "") or ("pytest" in sys.modules)
...
def load_serialized_clvm(
    ...
    # Set the SHIBGREEN_DEV_COMPILE_CLVM_ON_IMPORT environment variable to anything except
    # "" or "0" to trigger automatic recompilation of the SHIBgreenlisp on load.
    if recompile:
        try:
            if pkg_resources.resource_exists(package_or_requirement, clvm_filename):
                ...
                if include_standard_libraries:
                    ...
                    shibgreen_puzzles_path = pathlib.Path(pkg_resources.resource_filename(__name__, "__init__.py")).parent
                    search_paths.append(shibgreen_puzzles_path)
