...
p2_delegated_puzzle_or_hidden_puzzle is essentially the "standard coin" in shibgreen.
...
- The user's wallet contains a primary sk/pk pair which are used to derive to one
  ...
  https://shibgreenlisp.com/docs/standard_transaction. Therefore, when a wallet needs
...
- Python code previously appeared which was written like:
  ...
  In context, delegated_puzzle_solution here is any *shibgreenlisp program*, here one
  ...
  The second and third arguments are a shibgreenlisp program and its corresponding
...
from shibgreen.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
