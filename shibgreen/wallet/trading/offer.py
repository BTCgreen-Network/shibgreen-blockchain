...
from shibgreen.util.bech32m import bech32_encode, bech32_decode, convertbits
...
class Offer:
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        # Then we add a potential fee as pending XSHIB
        fee: int = sum(c.amount for c in all_removals) - sum(c.amount for c in all_additions)
        if fee > 0:
            pending_dict.setdefault("xshib", 0)
            pending_dict["xshib"] += fee
