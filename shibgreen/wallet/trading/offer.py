...
from shibgreen.util.bech32m import bech32_decode, bech32_encode, convertbits
from shibgreen.util.ints import uint64
from shibgreen.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    match_puzzle,
    solve_puzzle,
...
from shibgreen.wallet.util.puzzle_compression import (
    compress_object_with_puzzles,
    decompress_object_with_puzzles,
    lowest_best_version,
)
from shibgreen.wallet.uncurried_puzzle import UncurriedPuzzle, uncurry_puzzle
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting XSHIB
        coins: List[Coin],
    ...
    def get_offered_coins(self) -> Dict[Optional[bytes32], List[Coin]]:
        ...
        for parent_spend in self.bundle.coin_spends:
            ...
            if puzzle_driver is not None:
                ...
                # It's much easier if the asset is bare XSHIB
                asset_id = None
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "xshib" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
