...
from shibgreen.wallet.puzzles.puzzle_utils import (
...
from shibgreen.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_shibgreen(self, shibgreen_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if shibgreen_amount < 0:
            utxos = await self.select_coins(uint64(abs(shibgreen_amount)), exclude)
        else:
        ...
        shibgreen_amount = spend_value + shibgreen_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(shibgreen_amount), "memos": []}
                ]
    ...
    # WSSHIBgreenConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSHIBgreenConnection
