...
from shibgreen.wallet.db_wallet.db_wallet_puzzles import ACS_MU_PH
...
class TradeManager:
    ...
    By default, standard XSHIB is supported but to support other types of assets you must implement certain functions on
    ...
    Puzzle Drivers:
      - See shibgreen/wallet/outer_puzzles.py for a full description of how to build these
    ...
    async def coins_of_interest_farmed(
        self, coin_state: CoinState, fork_height: Optional[uint32], peer: WSSHIBgreenConnection
    ) -> None:
        """
    ...
    async def cancel_pending_offer_safely(
        ...
        for coin in Offer.from_bytes(trade.offer).get_cancellation_coins():
            ...
            # This should probably not switch on whether or not we're spending a XSHIB but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is XSHIB or
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not XSHIB
            ...
            for id, selected_coins in coins_to_offer.items():
                ...
                # This should probably not switch on whether or not we're spending XSHIB but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
                    tx = await wallet.generate_signed_transaction(
                        abs(offer_dict[id]),
                        Offer.ph(),
    ...
    async def check_offer_validity(self, offer: Offer, peer: WSSHIBgreenConnection) -> bool:
