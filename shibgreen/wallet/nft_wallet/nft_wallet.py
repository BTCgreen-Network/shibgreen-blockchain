...
from shibgreen.wallet.nft_wallet import nft_puzzles
from shibgreen.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from shibgreen.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from shibgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from shibgreen.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from shibgreen.wallet.payment import Payment
from shibgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from shibgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from shibgreen.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from shibgreen.wallet.transaction_record import TransactionRecord
from shibgreen.wallet.uncurried_puzzle import uncurry_puzzle
from shibgreen.wallet.util.compute_memos import compute_memos
from shibgreen.wallet.util.debug_spend_bundle import disassemble
from shibgreen.wallet.util.transaction_type import TransactionType
from shibgreen.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
from shibgreen.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSSHIBgreenConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSSHIBgreenConnection) -> None:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("SHIBgreen Signed Message", message))
    ...
    async def create_tandem_xshib_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        shibgreen_coins = await self.standard_wallet.select_coins(fee)
        shibgreen_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
            (await self.standard_wallet.get_new_puzzlehash()),
            fee=fee,
            coins=shibgreen_coins,
            coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
        )
        assert shibgreen_tx.spend_bundle is not None
        return shibgreen_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, shibgreen_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if shibgreen_tx is not None and shibgreen_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, shibgreen_tx.spend_bundle])
            shibgreen_tx = dataclasses.replace(shibgreen_tx, spend_bundle=None)
        ...
        if shibgreen_tx is not None:
            tx_list.append(shibgreen_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            shibgreen_tx = await self.create_tandem_xshib_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, shibgreen_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XSHIB'} to offer")
    ...
    async def mint_from_did(
        ...
        xshib_coins: Optional[Set[Coin]] = None,
        xshib_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xshib spend that funds the transactions and pays fees.
        ...
        :param xshib_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xshib_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xshib spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xshib coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xshib_coins is None:
            xshib_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xshib_coins) > 0
        ...
        # Create the xshib spend to fund the minting.
        spend_value = sum([coin.amount for coin in xshib_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xshib_spends = []
        if xshib_change_ph is None:
            xshib_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xshib_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xshib_change_ph, "amount": change, "memos": [xshib_change_ph]})
        ]

        first = True
        for xshib_coin in xshib_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xshib_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xshib_coins]
                message_list.append(
                    Coin(xshib_coin.name(), xshib_primaries[0]["puzzlehash"], xshib_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xshib_coins) > 1:
                    xshib_announcement: Optional[Set[bytes]] = {message}
                else:
                    xshib_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xshib_primaries,
                    fee=fee,
                    coin_announcements=xshib_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xshib_coin.name(), message).name()
            ...
            xshib_spends.append(CoinSpend(xshib_coin, puzzle, solution))
        xshib_spend = await self.standard_wallet.sign_transaction(xshib_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xshib_spend, *eve_spends])
        return total_spend

    async def mint_from_xshib(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XSHIB spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xshib_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XSHIB to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XSHIB spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XSHIB spend
            ...
            # Annnouncements for eve spend. These are asserted by the xshib spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xshib_coin in xshib_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xshib_primaries + primaries,
                    fee=fee,
                    coin_announcements=xshib_announcement if len(xshib_coins) > 1 else None,
...
if TYPE_CHECKING:
    from shibgreen.wallet.wallet_protocol import WalletProtocol
