...
from shibgreen.daemon.server import service_launch_lock_path
...
from shibgreen.rpc.rpc_server import RpcApiProtocol, RpcServiceProtocol, start_rpc_server, RpcServer
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        self._log.info(f"shibgreen-blockchain version: {shibgreen_full_version_str()}")
        ...
        self._server = SHIBgreenServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"shibgreen_{self._service_name}"
