...
from shibgreen.util.api_decorators import get_metadata
from shibgreen.util.errors import Err, ProtocolError
from shibgreen.util.ints import uint16
from shibgreen.util.network import WebServer, is_in_network, is_localhost
...
class SHIBgreenServer:
    ...
    all_connections: Dict[bytes32, WSSHIBgreenConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        shibgreen_ca_crt_path, shibgreen_ca_key_path = shibgreen_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                shibgreen_ca_crt_path, shibgreen_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                shibgreen_ca_crt_path, shibgreen_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("SHIBgreenServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        try:
            connection = WSSHIBgreenConnection.create(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    def connection_closed(self, connection: WSSHIBgreenConnection, ban_time: int, closed_connection: bool = False):
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        try:
            ...
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.shib.green/") as resp:
                    if resp.status == 200:
