...
from shibgreen.server.address_manager_sqlite_store import create_address_manager_from_db
from shibgreen.server.outbound_message import NodeType, make_msg, Message
...
class FullNodeDiscovery:
    ...
    async def update_peer_timestamp_on_message(self, peer: WSSHIBgreenConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: WSSHIBgreenConnection):
    ...
    async def on_connect_callback(self, peer: WSSHIBgreenConnection):
