...
from shibgreen.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from shibgreen.cmds.peer_funcs import print_connections
from shibgreen.cmds.units import units
from shibgreen.rpc.wallet_rpc_client import WalletRpcClient
from shibgreen.server.start_wallet import SERVICE_NAME
from shibgreen.types.blockchain_format.sized_bytes import bytes32
from shibgreen.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from shibgreen.util.config import load_config, selected_network_address_prefix
from shibgreen.util.default_root import DEFAULT_ROOT_PATH
from shibgreen.util.ints import uint16, uint32, uint64
from shibgreen.wallet.nft_wallet.nft_info import NFTInfo
from shibgreen.wallet.outer_puzzles import AssetType
from shibgreen.wallet.puzzle_drivers import PuzzleInfo
...
from shibgreen.wallet.util.address_type import AddressType, ensure_valid_address
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        shibgreen_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {shibgreen_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    print(f"To get status, use command: shibgreen wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["shibgreen"])
        ...
        for item in [*offers, *requests]:
                        ...
                        if item in requests:
                            ...
                            if info.supports_did:
                                ...
                                royalty_asset_dict[name] = (
                                    encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.XSHIB.hrp(config)),
            ...
            if royalty_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    ...
                    for summary in summaries:
                        divisor = units["shibgreen"] if summary["asset"] == "XSHIB" else units["cat"]
                ...
                for asset, requested_amount in total_amounts_requested.items():
                    divisor = units["shibgreen"] if asset == "XSHIB" else units["cat"]
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use shibgreen wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_offer_summary(cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False):
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["shibgreen"]
        wid: str = "1" if asset_id == "xshib" else ""
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['shibgreen']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
        if royalty_asset_id.hex() in offered:
            ...
            royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                encode_puzzle_hash(address, AddressType.XSHIB.hrp(config)),
                percentage,
            )
    ...
    if royalty_asset_dict != {}:
        ...
        for fungible_asset_id in fungible_assets_from_offer(offer):
            fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "xshib"
            if fungible_asset_id_str in requested:
                nft_royalty_currency: str = "Unknown CAT"
                if fungible_asset_id is None:
                    nft_royalty_currency = "XSHIB"
                else:
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['shibgreen']}")
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use shibgreen wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use shibgreen wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["shibgreen"]
    return "", units["mojo"]
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.XSHIB}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.XSHIB}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.XSHIB}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["shibgreen"])
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["shibgreen"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["shibgreen"])

    tx = await wallet_client.send_notification(address, message, amount, fee)

    print("Notification sent successfully.")
    print(f"To get status, use command: shibgreen wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def sign_message(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    if args["type"] == AddressType.XSHIB:
