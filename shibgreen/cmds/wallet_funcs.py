...
from shibgreen.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
from shibgreen.cmds.show import print_connections
...
from shibgreen.wallet.nft_wallet.nft_info import NFT_HRP, NFTInfo
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["shibgreen"]))
    final_amount: uint64
    final_min_coin_amount: uint128
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["shibgreen"]))
        final_min_coin_amount = uint128(int(min_coin_amount * units["shibgreen"]))
    ...
    print(f"Do 'shibgreen wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        if nft_royalty_asset_id == "shibgreen":
        ...
        nft_royalty_divisor = units["shibgreen"] if nft_royalty_asset_id == "shibgreen" else units["cat"]
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'shibgreen keys generate' or import a key")
        return None
