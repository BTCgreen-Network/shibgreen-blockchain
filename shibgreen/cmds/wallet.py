...
from shibgreen.cmds.cmds_util import execute_with_wallet
from shibgreen.cmds.coins import coins_cmd
...
@wallet_cmd.command("send", short_help="Send shibgreen to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much shibgreen to send, in XSHIB", type=str, required=True)
...
@click.option("-t", "--address", help="Address to send the XSHIB", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much XSHIB or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much XSHIB or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XSHIB}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XSHIB/CATs for XSHIB/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XSHIB.",
    type=str,
    default="0",
    show_default=True,
