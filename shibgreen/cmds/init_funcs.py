...
from shibgreen.cmds.configure import configure
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xshib_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xshib_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xshib_target_address") or all_targets[-2] == config["pool"].get(
                    "xshib_target_address"
                ):
        ...
        if "xshib_target_address" not in config["farmer"]:
            print(
                f"Setting the xshib destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def init(
    ...
    if create_certs is not None:
        ...
        return shibgreen_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def shibgreen_init(
    ...
    *,
    ...
    """
    shibgreen_root = os.environ.get("SHIBGREEN_ROOT", None)
    if shibgreen_root is not None:
        print(f"SHIBGREEN_ROOT is set to {shibgreen_root}")
