...
from shibgreen.cmds.passphrase_funcs import obtain_current_passphrase
...
from shibgreen.util.errors import KeychainException, KeychainNotSet
from shibgreen.util.file_keyring import MAX_LABEL_LENGTH
from shibgreen.util.ints import uint32
from shibgreen.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def generate_and_print():
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run shibgreen keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'shibgreen keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from shibgreen.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using SHIBgreen.\n"
    )
    ...
    if Keychain.needs_migration():
        ...
        if len(keys_to_migrate) > 0:
            ...
            if not prompt_yes_no("Migrate these keys?"):
                ...
                print("Migration aborted, can't run any shibgreen commands.")
                return False
