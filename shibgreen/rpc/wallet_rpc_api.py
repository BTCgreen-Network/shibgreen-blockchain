...
from shibgreen.data_layer.data_layer_wallet import DataLayerWallet
from shibgreen.pools.pool_wallet import PoolWallet
from shibgreen.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from shibgreen.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from shibgreen.types.blockchain_format.coin import Coin, coin_as_list
...
from shibgreen.util.errors import KeychainIsLocked
from shibgreen.util.ints import uint8, uint32, uint64, uint16
from shibgreen.util.keychain import bytes_to_mnemonic, generate_mnemonic
from shibgreen.util.path import path_from_root
from shibgreen.util.ws_message import WsRpcMessage, create_payload_dict
from shibgreen.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from shibgreen.wallet.cat_wallet.cat_wallet import CATWallet
...
from shibgreen.wallet.did_wallet.did_wallet_puzzles import match_did_puzzle, program_to_metadata
from shibgreen.wallet.nft_wallet import nft_puzzles
from shibgreen.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from shibgreen.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from shibgreen.wallet.nft_wallet.nft_wallet import NFTWallet
from shibgreen.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from shibgreen.wallet.notification_store import Notification
from shibgreen.wallet.outer_puzzles import AssetType
from shibgreen.wallet.puzzle_drivers import PuzzleInfo, Solver
from shibgreen.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from shibgreen.wallet.trade_record import TradeRecord
from shibgreen.wallet.trading.offer import Offer
...
from shibgreen.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def get_latest_coin_spend(
        self, peer: Optional[WSSHIBgreenConnection], coin_id: bytes32, latest: bool = True
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from shibgreen.util.bech32m import bech32_decode, convertbits
        from shibgreen.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSSHIBgreenConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xshib")
        ...
        key = None
        if asset_id is not None and asset_id != "xshib":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XSHIB.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xshib_coin_list = request.get("xshib_coins", None)
        xshib_coins = None
        if xshib_coin_list:
            xshib_coins = set([Coin.from_json_dict(xshib_coin) for xshib_coin in xshib_coin_list])
        xshib_change_target = request.get("xshib_change_target", None)
        if xshib_change_target is not None:
            if xshib_change_target[:2] == "xshib":
                xshib_change_ph = decode_puzzle_hash(xshib_change_target)
            else:
                xshib_change_ph = bytes32(hexstr_to_bytes(xshib_change_target))
        else:
            xshib_change_ph = None
        ...
        if mint_from_did:
            sb = await nft_wallet.mint_from_did(
                ...
                xshib_coins=xshib_coins,
                xshib_change_ph=xshib_change_ph,
            ...
            sb = await nft_wallet.mint_from_xshib(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
