...
from shibgreen.rpc.util import wrap_http_handler
from shibgreen.server.outbound_message import NodeType
from shibgreen.server.server import SHIBgreenServer, ssl_context_for_client, ssl_context_for_server
...
from shibgreen.util.config import str2bool
...
from shibgreen.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> SHIBgreenServer:
        ...
        # a property so as to be read only which allows SHIBgreenServer to satisfy
        # Optional[SHIBgreenServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        shibgreen.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSSHIBgreenConnection) -> None:
...
def default_get_connections(server: SHIBgreenServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
