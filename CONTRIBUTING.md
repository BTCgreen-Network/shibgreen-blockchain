...
If you want to learn more about this project, read the [wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki), or check out the [green paper](https://www.shib.green/assets/SHIBgreenGreenPaper.pdf).
...
In the event an emergency fix is required for the release version of SHIBgreen, members of the SHIBgreen organization will create a feature branch from the current release branch `latest`.
...
sh install.sh -d
. ./activate
black . && isort benchmarks build_scripts shibgreen tests tools *.py && mypy && flake8 benchmarks build_scripts shibgreen tests tools *.py && pylint benchmarks build_scripts shibgreen tests tools *.py
...
The current official testnet is testnet10. Look at `shibgreen/util/initial_config.yaml` to see the configuration parameters
