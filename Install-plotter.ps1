...
function get_bladebit_url()
{
    ...
    $GITHUB_BASE_URL = "https://github.com/SHIBgreen-Network/bladebit/releases/download"
...
function get_madmax_filename()
{
    ...
    $shibgreen_plot = "shibgreen_plot"
    if ("${ksize}" -eq "k34")
    {
        $shibgreen_plot = "shibgreen_plot_k34"
    }
    $suffix = ""
    if ("${os}" -eq "macos")
    {
...
function get_madmax_url()
{
    ...
    $GITHUB_BASE_URL = "https://github.com/SHIBgreen-Network/chia-plotter-madmax/releases/download"
...
try {
    ...
    elseif("${plotter}" -eq "madmax")
    {
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "shibgreen_plot.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax to $(Get-Location)\shibgreen_plot.exe"
        }
        catch {
            Write-Output "ERROR: Download failed. Maybe specified version of the binary does not exist."
            Pop-Location
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "shibgreen_plot_k34.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax for k34 to $(Get-Location)\shibgreen_plot_k34.exe"
        }
        catch {
