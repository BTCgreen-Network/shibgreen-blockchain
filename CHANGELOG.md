...
- New wallet CLI option `shibgreen wallet sign_message`
...
- SHIBgreenvdf updated to 1.0.7
- SHIBgreenpos updated to 1.0.11
- Clvm_tools updated to 0.4.5
- SHIBgreen_rs updated to 0.1.14
...
- Removed "Total iterations since start" from `shibgreen show -s`
- Removed rate-limited wallet
- Removed the beta program link from the warning in the CLI
- Removed `--enable-data-server` from `shibgreen configure` CLI
...
- Dropped unused `shibgreen_minor_release_number`
- Just `raise`, not `raise e` when reraising
- Optimized `simple_solution_generator()`
- Allow developers to easily use standard SHIBgreen `clvm` puzzles and libraries
- Skipped validating `GTElement` in mempool
- Improved logging for `shibgreen plotters version` errors
...
- Minter DID now shown in output of `shibgreen wallet nft get_info` and in GUI
...
- Deprecated `shibgreen plotters install` command
...
- Fixed shibgreen farm summary if using a remote full node (thanks @yan74)
...
- Fixed the run_block utility to use shibgreenlisp_deserialization
...
- Fixed `shibgreen show -s` with other options as well
...
- Show Usage when running `shibgreen plotters` (#13690)
...
- Fixed issues where SHIBgreenLisp was compiled during import requiring write access to the directory (#11257) (thanks @lourkeur). To force compilation, developers can set environment variable `SHIBGREEN_DEV_COMPILE_CLVM_ON_IMPORT`
...
## 1.6.0 SHIBgreen blockchain 2022-9-20
...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/SHIBgreen-Network/shibgreen-blockchain-gui/issues/950)

### Known Issues

- The CLI command `shibgreen configure --enable-data-server`, and the `config.yaml` parameter at `data_layer.run_server` have no effect, and will be removed in the future
...
## 1.5.1 SHIBgreen blockchain 2022-8-23

### Added

- Add Source and Changelog to project_urls (Thanks @strayer!)
- Add condition code constant for REMARK, an always true SHIBgreenlisp condition
- Add several wallet optimizations
- Add `shibgreen db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the shibgreenlisp code when necessary
...
- Remove shibgreen.util.path.mkdir()
...
- Removing `logging.basicConfig()` from `SHIBgreenServer.__init__()`
...
- Log `shibgreen_full_version_str` in daemon and all services
...
- Substantially reduced overall size of SHIBgreen packages
...
- Make sure shibgreen commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 SHIBgreen blockchain 2022-7-26
...
- Update `shibgreen wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the SHIBgreen blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the SHIBgreen blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 SHIBgreen blockchain 2022-6-29

### Added

- Added support for NFTs!!! :party:
- Added `shibgreen wallet nft` command (see <https://docs.shib.green/docs/13cli/did_cli>)
- Added `shibgreen wallet did` command (see <https://docs.shib.green/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.shib.green/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.shib.green/docs/12rpcs/nft_rpcs>)
...
- Added CLI options `--sort-by-height` and –sort-by-relevance` to `shibgreen wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously shibgreen selected the largest coin
...
- Updated warning about `SHIBGREEN_ROOT` being set when running init
...
## 1.3.5 SHIBgreen blockchain 2022-5-11
...
## 1.3.4 SHIBgreen blockchain 2022-4-19
...
- Implemented `shibgreen rpc` command
...
- Added `-n`/`--new-address` option to `shibgreen wallet get_address`
...
- Fixed typo in command line argument parsing for `shibgreen db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `shibgreen configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `shibgreen db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `shibgreen show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `shibgreen wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `shibgreen wallet show`
...
- You cannot install and run shibgreen blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 SHIBgreen blockchain 2022-4-02
...
- Fixed exception when `shibgreen keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `xshib` or `txshib`.
...
## 1.3.0 SHIBgreen blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of SHIBgreen (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores SHIBgreen will not use when launching process pools. Using 0 will allow SHIBgreen to use all cores for process pools. Set the default to 0 to allow SHIBgreen to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, shibgreen keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using shibgreen keys derive search.
...
- CLI option shibgreen show will display the currently selected network (mainnet or testnet).
- CLI option shibgreen plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with shibgreen show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the shibgreen-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI shibgreen show -c when No Info should be displayed.
...
- When you close the SHIBgreen app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the SHIBgreen and relaunch the SHIBgreen app.
...
- Farmers rejoice: today's release integrates two plotters in broad use in the SHIBgreen community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/SHIBgreen-Network/shibgreen-blockchain/pull/9050).
...
We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Passphrase-Protected-SHIBgreen-Keys-and-Key-Storage-Migration). We also launched a new SHIBgreenlisp compiler in clvm_tools_rs which substantially improves compile time for SHIBgreenlisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Passphrase-Protected-SHIBgreen-Keys-and-Key-Storage-Migration).
...
## 1.2.8 SHIBgreen blockchain 2021-09-30
...
- Beta builds are built on every merge to main, and are now available from <https://shib.green/download/>.
...
- We have made many full node changes to support our upcoming SHIBgreen Asset Token (CAT) standard and our upcoming standalone light wallet, which will use SHIBgreen's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming SHIBgreen Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install shibgreen-blockchain to type check using our functions with mypy.
...
- Thanks @altendky for changing the default to paginate to shibgreen wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use SHIBgreen's service ip.shib.green.
...
## 1.2.3 SHIBgreen blockchain 2021-07-26
...
## 1.2.2 SHIBgreen blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/SHIBgreen-Network/shibgreen-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
## 1.2.0 SHIBgreen blockchain 2021-07-07
...
- All SHIBgreen repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `shibgreen plotnft` to manage pools.
...
- Correctly display private key in `shibgreen keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `shibgreen keys show`.
...
- Thank you to @SHIBgreenMineJP for various improvements.
...
- `shibgreen farm summary` better handles wallet errors.
...
### 1.1.7 SHIBgreen Blockchain 2021-06-05
...
## 1.1.6 SHIBgreen Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /shibgreen/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `shibgreen configure -t true|false`).
...
## 1.1.5 SHIBgreen Blockchain 2021-05-09
...
## 1.1.4 SHIBgreen Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `shibgreen keys show`. Use `shibgreen keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 SHIBgreen Blockchain 2021-05-01
...
## 1.1.2 SHIBgreen Blockchain 2021-04-24
...
## 1.1.1 SHIBgreen Blockchain 2021-04-21
...
## 1.1.0 SHIBgreen Blockchain 2021-04-21
...
## 1.0.5 SHIBgreen Blockchain 2021-04-14
...
## 1.0.4 SHIBgreen Blockchain 2021-04-12
...
- The `src` directory in shibgreen-blockchain has been changed to `shibgreen` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `shibgreen/`
...
## 1.0.3 SHIBgreen Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/SHIBgreen-Network/shibgreen-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `shibgreen keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 SHIBgreen Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/SHIBgreen-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- @jespino changed `shibgreen keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `shibgreen keys show`.
...
## 1.0.1 SHIBgreen Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `shibgreen keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/shibgreen-blockchain/).
...
- Users can now pip install e.g. shibgreen-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of SHIBgreen Blockchain 2021-03-17

### Added

- This is the first production release of the SHIBgreen Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable XSHIB. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/shibgreen-blockchain).
...
- Migration with SHIBGREEN_ROOT set does not crash shibgreen init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXSHIB Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.shib.green/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `shibgreen farm summary` will now display TXSHIB or XSHIB as appropriate.
...
- `shibgreen farm summary` displayed some incorrect amounts.
...
- `shibgreen netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `shibgreen plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `shibgreen keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We have moved to a new and much higher scalability download.shib.green to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.shib.green/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.shib.green/latest/Setup-MacOS.dmg).
...
- The Farm page will now no longer get stuck at 50 TXSHIB farmed.
- `shibgreen farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `shibgreen wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xshib_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TXSHIB will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.shib.green/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `shibgreen farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of shibgreen-blockchain, etc., instead of in blspy.
...
- Adding Click changed the behavior of `shibgreen keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.shib.green via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the shibgreen sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all SHIBgreen dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `shibgreen -upnp f` was added to disable uPnP.
- `shibgreen plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/SHIBgreen-Network/shibgreen-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `shibgreen -h` and `shibgreen SUBCOMMAND -h` can be your guide.
...
- Interface improvements for `shibgreen netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `shibgreen init` only migrates release candidate directories. The versioned sub directories under `~/shibgreen` will be going away before mainnet.
...
- `shibgreen configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/SHIBgreen-Network/shibgreen-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/SHIBgreen-Network/shibgreen-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- The Beta 27 chain is a hard fork. All TXSHIB from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `shibgreen wallet -h` to learn more. Also, `shibgreen wallet` now requires a third argument of `show`, therefor you will use `shibgreen wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [shibgreen blockchain gui](https://crowdin.com/project/shibgreen-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `shibgreen init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `shibgreen init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from shibgreen-blockchain, [shibgreen-blockchain-gui](https://github.com/SHIBgreen-Network/shibgreen-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `shibgreen-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `shibgreen wallet`, was incorrectly displaying TXSHIB balances as if they were Coloured Coins.
...
- `shibgreen plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.

### Changed

- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TXSHIB as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/SHIBgreen-Network/clvm_rs) is complete.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/SHIBgreen-Network/shibgreen-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `shibgreen plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and SHIBgreenLisp code matches the CI compiled code.
- We have removed the '-r' flag from `shibgreen` as it was being overridden in most cases by the `-r` for restart flag to `shibgreen start`. Use `shibgreen --root-path` instead.
- `shibgreen -h` now recommends `shibgreen netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `shibgreen show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `shibgreen configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `shibgreen plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `shibgreen plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `shibgreen show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `shibgreen show` handles sub block 0 better.
...
- `shibgreen plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `shibgreen plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `shibgreen show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `shibgreen configure --enable-upnp`.
...
- `shibgreen configure` now lets you enable or disable uPnP.
...
- `shibgreen netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `shibgreen show` of the GUI use `shibgreen netspace -d 1000` as `shibgreen netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TXSHIB. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TXSHIB compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TXSHIB.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [shib.green](https://shib.green/). Among the improvements this gives the SHIBgreen blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TXSHIB from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TXSHIB to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TXSHIB to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `shibgreen plots create`. After obtaining the fingerprint from `shibgreen keys show`, try `shibgreen plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `shibgreen configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `shibgreen configure -h`. Note that if shibgreen services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Tests now place a cache of blocks and plots in the ~/.shibgreen/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run shibgreen-blockchain.
...
- `shibgreen show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/SHIBgreen-Network/shibgreen-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/SHIBgreen-Network/shibgreen-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/SHIBgreen-Network/shibgreen-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/SHIBgreen-Network/shibgreen-blockchain/tree/dev/electron-react/src/locales) of the shibgreen-blockchain repository.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/SHIBgreen-Network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all shibgreen services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test shibgreen since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve shibgreen-blockshibgreenn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from SHIBgreen Network at backup.shib.green. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the SHIBgreen backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.shib.green, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test shibgreen are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.shibgreen` directory.
...
- `shibgreen init` now automatically discovers previous releases in each new release.

### Fixed

- `shibgreen show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `shibgreen show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now SHIBgreen Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The SHIBgreen UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (shibgreen run farmer-only, wallet-only, timelord-only)
...
- `shibgreen show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `shibgreen show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the SHIBgreen simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the SHIBgreen implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test shibgreen between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test shibgreen from the earlier two releases however.
...
- `shibgreen show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching SHIBgreen)
...
- See wallet balances in command line: `shibgreen show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `shibgreen keys sign` and `shibgreen keys verify` to allow farmers to certify their ownership of keys.
...
- `shibgreen start wallet-server` changed to `shibgreen start wallet`, for consistency.
...
- Removed legacy scripts such as shibgreen-stop-server, shibgreen-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/SHIBgreen-Network/bls-signatures) to use the
...
- The shibgreen executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./shibgreen -h` from
`~\AppData\Local\SHIBgreen-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/SHIBgreen.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed XSHIB units to TXSHIB units for testnet.
...
"shibgreen plots create" and "shibgreen plots check".
- Add plot directories to config.yaml from the cli with "shibgreen plots add".
...
- The Windows installer is now signed by a SHIBgreen Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.shibgreen/test-plots/`
- We have completely refactored all networking code towards making each SHIBgreen service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make shibgreen start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the SHIBgreen Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting shibgreen now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'shibgreen keys' command line.
...
- We added a `shibgreen keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the shibgreen-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `shibgreen netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `shibgreen-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `shibgreen-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `shibgreen-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `shibgreen-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `shibgreen init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `shibgreen start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `shibgreen-start-harvester`.
...
- The user interface now refers to shibgreen coins with their correct currency code of XSHIB.
...
- There was a regression in SHIBgreen Proof of Space ([chiapos](https://github.com/SHIBgreen-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `shibgreen init` then sync on first start will happen automatically. "\$SHIBGREEN_ROOT" users will need to delete `$SHIBGREEN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `shibgreen netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki) and placed in [INSTALL](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Quick-Start-Guide)
...
- We suggest you take a look at our [Upgrading documentation](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `shibgreen start wallet-gui` has been chaned to `shibgreen start wallet` which launches but the GUI and server on MacOS and Linux. `shibgreen start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `shibgreen stop all` command sometimes fails, use `shibgreen-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `shibgreen init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `shibgreen-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/SHIBgreen-Network/shibgreen-blockchain/wiki/Updating-beta-software). If you've set `$SHIBGREEN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `shibgreen show`.

### Changed

- `shibgreen-restart-harvester` has been renamed from `shibgreen-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.shibgreen/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the SHIBgreen blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/SHIBgreen-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the shibgreen management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `shibgreen-start-farmer &`. The [README.md](https://github.com/SHIBgreen-Network/shibgreen-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries shibgreen-pos, shibgreen-fast-vdf, and shibgreen-bip-158 have been moved to their own repositories: [chiapos](https://github.com/SHIBgreen-Network/chiapos), [chiavdf](https://github.com/SHIBgreen-Network/chiavdf), and [chaibip158](https://github.com/SHIBgreen-Network/chiabip158). They are brought in by shibgreen-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/SHIBgreen-Network/bls-signatures).
...
- SHIBgreen fullnode, farmer and harvester now default to logging to shibgreen.log in the shibgreen-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the SHIBgreen blockchain.
- Implemented an RPC interface with JSON serialization for streamables - currently on port 18793.
- Added details on how to contribute in CONTRIBUTING.md. Thanks @RichardLitt.
- Added color logging
- Now shibgreen_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to shibgreen_full node, shibgreen_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new shibgreen services were started.
...
- More details on the release at [https://www.shib.green/developer/](https://www.shib.green/developer/)

[unreleased]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/SHIBgreen-Network/shibgreen-blockchain/releases/tag/Alpha-1.0
